...
import org.apache.storm.ql.Driver;
import org.apache.storm.ql.exec.Utils;
import org.apache.storm.ql.exec.Utils.StreamPrinter;
import org.apache.storm.ql.parse.BoltDesc;
import org.apache.storm.ql.parse.Context;
import org.apache.storm.ql.parse.SpoutDesc;
import org.apache.storm.ql.parse.VariableSubstitution;
import org.apache.storm.ql.processors.CommandProcessor;
import org.apache.storm.ql.processors.CommandProcessorFactory;
...
public class StormCli {
	...
	private int run(String[] args) throws Exception {
		...
		CliSessionState ss = new CliSessionState(new StormConf(
				SessionState.class));
		ss.in = System.in;
	...
	private int executeDriver(CliSessionState ss, StormConf conf,
		...
		if (ss.execString != null) {
			int cmdProcessStatus = cli.processLine(ss.execString, context,
					false);
	...
	private int processLine(String line, Context context,
			boolean allowInterupting) throws Exception {
	...
	public int processCmd(String cmd, Context context) throws Exception {
		...
		if (cmd_trimmed.toLowerCase().equals("quit")
			...
			for (Map.Entry<String, SpoutDesc> s : context.getSpoutDescs()
					.entrySet()) {
			...
			for (Map.Entry<String, BoltDesc> b : context.getBoltDescs()
					.entrySet()) {
				...
				if (type.equals("SHUFFLE")) {
					builder.setBolt(boltAlias, (IRichBolt) bolt.getBolt(),
							bolt.getParallelismHint()).shuffleGrouping(
							bolt.getComponentId());
				}
				if (type.equals("FIELDS")) {
					builder.setBolt(boltAlias, (IBasicBolt) bolt.getBolt(),
							bolt.getParallelismHint()).fieldsGrouping(
							bolt.getComponentId(), bolt.getFields());
				}
			}
			String topologyName = (String) conf.get(Config.TOPOLOGY_NAME);
			Integer workerNums = backtype.storm.utils.Utils.getInt(
			...
			try {
				...
				Utils.StreamPrinter outPrinter = new StreamPrinter(
						executor.getInputStream(), null, ss.out);
				StreamPrinter errPrinter = new StreamPrinter(
						executor.getErrorStream(), null, ss.err);
			...
			try {
				...
				if (proc != null) {
					if (proc instanceof Driver) {
						...
						console.printInfo("Time taken: " + timeTaken
								+ " seconds");
					} else {
